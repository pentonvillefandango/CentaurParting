...
#
# Model test for psf_basic_metrics.
# - Builds SELECT list dynamically from actual psf_basic_metrics schema
# - Single LEFT JOIN base query (prevents losing rows)
# - Worker-run breakdown for psf_basic_worker:
#     * missing_worker_run
#     * worker_failed
...
#     data/model_tests/psf_basic_metrics/test_results_<STAMP>/
#   Master run:
#     --run-dir data/model_tests/test_results_<STAMP>
#       writes into: <run-dir>/psf_basic_metrics/
#
# Exit codes:
#   0 PASS
#   2 FAIL
...
TABLE_NAME = "psf_basic_metrics"
WORKER_NAME = "psf_basic_worker"
...
def _check_one_row(r: Dict[str, Any], metric_cols: set[str]) -> List[str]:
    ...
    # Basic psf sanity: if present, these should be positive
    for c in (
        "fwhm_px_median",
        "fwhm_px_mean",
        "fwhm_px_p50",
    ...
    # Eccentricity fields usually in [0,1]
    for c in ("ecc_median", "ecc_mean", "ecc_p50", "ecc_p90"):
        if c in metric_cols:
            v = _safe_float(_m(r, c))
            if v is None:
                _add_issue(issues, f"{c}_missing")
    ...
    # Counts should be non-negative if present
    for c in ("n_stars_used", "n_stars", "n_sources"):
        if c in metric_cols:
            v = _safe_int(_m(r, c))
            if v is None:
                _add_issue(issues, f"{c}_missing")
...
def _rollup(rows: List[Dict[str, Any]], metric_cols: set[str]) -> List[Dict[str, Any]]:
    ...
    def pick(r: Dict[str, Any]) -> Optional[float]:
        # Prefer n_stars_used if present
        for c in ("n_stars_used", "n_stars", "n_sources"):
            if c in metric_cols:
                v = _safe_float(_m(r, c))
                if v is not None:
        ...
        out.append(
            {
                ...
                "avg_stars_like": (
                    None
                    if avg(g["_stars_like"]) is None
                    else round(avg(g["_stars_like"]), 3)
                ),
            }
        )
    return out
