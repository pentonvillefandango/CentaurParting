...
#
# Model test for psf_grid_metrics.
# - Builds SELECT list dynamically from actual psf_grid_metrics schema
# - Single LEFT JOIN base query (prevents losing rows)
# - Worker-run breakdown for psf_grid_worker:
#     * missing_worker_run
#     * worker_failed
...
#     data/model_tests/psf_grid_metrics/test_results_<STAMP>/
#   Master run:
#     --run-dir data/model_tests/test_results_<STAMP>
#       writes into: <run-dir>/psf_grid_metrics/
#
# Exit codes:
#   0 PASS
#   2 FAIL
...
TABLE_NAME = "psf_grid_metrics"
WORKER_NAME = "psf_grid_worker"
...
def _check_one_row(r: Dict[str, Any], metric_cols: set[str]) -> List[str]:
    ...
    # psf_grid is usually "grid-ish" dims. If present, validate.
    ...
    # cell size / spacing / tile size often positive
    for c in ("tile_size_px", "cell_size_px", "spacing_px"):
        if c in metric_cols:
            v = _safe_float(_m(r, c))
            if v is not None and v <= 0:
                _add_issue(issues, f"{c}_nonpos")

    # Fractions (if present) should be 0..1
    for c in ("nan_fraction", "inf_fraction", "clipped_fraction", "usable_fraction"):
        if c in metric_cols:
            v = _safe_float(_m(r, c))
