...
#
# Model test for psf_model_metrics.
# - Builds SELECT list dynamically from actual psf_model_metrics schema
# - Single LEFT JOIN base query (prevents losing rows)
# - Worker-run breakdown for psf_model_worker:
#     * missing_worker_run
#     * worker_failed
...
#     data/model_tests/psf_model_metrics/test_results_<STAMP>/
#   Master run:
#     --run-dir data/model_tests/test_results_<STAMP>
#       writes into: <run-dir>/psf_model_metrics/
#
# Exit codes:
#   0 PASS
#   2 FAIL
...
TABLE_NAME = "psf_model_metrics"
WORKER_NAME = "psf_model_worker"
...
def _required_schema_checks(conn: sqlite3.Connection) -> Tuple[bool, List[str]]:
    ...
    # Core fields expected in metrics table
    for col in (
        "image_id",
        "expected_fields",
        "read_fields",
...
def _check_one_row(r: Dict[str, Any], metric_cols: set[str]) -> List[str]:
    ...
    # Typical PSF model outputs: if present, ensure sane
    # (We don't assume exact column names; we apply rules when they exist.)

    # counts should be >=0
    for c in ("n_stars_used", "n_stars", "n_samples", "n_inliers", "n_outliers"):
        if c in metric_cols:
            v = _safe_int(_m(r, c))
            if v is None:
    ...
    # fwhm / sigma / width like fields should be >0 if present
    for c in ("fwhm_px", "fwhm_px_median", "sigma_px", "sigma_x_px", "sigma_y_px"):
        if c in metric_cols:
            v = _safe_float(_m(r, c))
            if v is not None and v <= 0:
                _add_issue(issues, f"{c}_nonpos")

    # ellipticity/eccentricity bounds if present
    for c in ("ecc", "ecc_median", "eccentricity", "ellipticity"):
        if c in metric_cols:
            v = _safe_float(_m(r, c))
    ...
    for c in ("nan_fraction", "inf_fraction", "clipped_fraction", "inlier_fraction"):
        if c in metric_cols:
            v = _safe_float(_m(r, c))
            if v is not None and not (0.0 <= v <= 1.0):
                _add_issue(issues, f"{c}_out_of_range")
...
def main() -> int:
    ...
    result: Dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "fail_if_worker_ok_but_missing_pct": (
                float(args.fail_if_worker_ok_but_missing_pcnt)
                if hasattr(args, "fail_if_worker_ok_but_missing_pcnt")
                else float(args.fail_if_worker_ok_but_missing_pct)
            ),
        },
    ...
    # Fix typo guard cleanly (ensure threshold exists and correct)
    result["thresholds"]["fail_if_worker_ok_but_missing_pct"] = float(
