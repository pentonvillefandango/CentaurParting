...
#     data/model_tests/nebula_mask_metrics/test_results_<STAMP>/
#   Master run:
#     --run-dir data/model_tests/test_results_<STAMP>
#       writes into: <run-dir>/nebula_mask_metrics/
#
# Exit codes:
#   0 PASS
#   2 FAIL
...
TABLE_NAME = "nebula_mask_metrics"
WORKER_NAME = "nebula_mask_worker"
...
SELECT
  ...
  m.mask_pixel_count,
  m.mask_coverage_frac,
  m.n_components,
  m.largest_component_frac,
  m.largest_component_bbox_json,
...
def _required_schema_checks(conn: sqlite3.Connection) -> Tuple[bool, List[str]]:
    ...
    # Core nebula_mask_metrics columns we rely on for deep checks
    for col in (
        "image_id",
        "expected_fields",
        "read_fields",
...
def _check_one_row(r: Dict[str, Any]) -> List[str]:
    ...
    # Worker ran but failed?
    if has_worker_run and str(worker_status).upper() != "OK":
        _add_issue(issues, "worker_failed")

    # expected/read/written sanity
    ...
    # parameter sanity: should be positive if present
    for name in ("threshold_sigma", "smooth_sigma_px", "bg_clip_sigma"):
        v = _safe_float(r.get(name))
        if v is None or v <= 0:
            _add_issue(issues, f"{name}_missing_or_nonpos")

    bg_maxiters = _safe_int(r.get("bg_clip_maxiters"))
    if bg_maxiters is None or bg_maxiters <= 0:
        _add_issue(issues, "bg_clip_maxiters_missing_or_nonpos")

    # background stats
    bg_median = _safe_float(r.get("bg_median_adu"))
    bg_mad = _safe_float(r.get("bg_madstd_adu"))
    thr_adu = _safe_float(r.get("threshold_adu"))
    if bg_median is None:
        _add_issue(issues, "bg_median_adu_missing")
    if bg_mad is None:
        _add_issue(issues, "bg_madstd_adu_missing")
    elif bg_mad < 0:
        _add_issue(issues, "bg_madstd_adu_negative")
    if thr_adu is None:
        _add_issue(issues, "threshold_adu_missing")

    # threshold consistency: threshold_adu should be >= bg_median (usually bg + k*sigma)
    if bg_median is not None and thr_adu is not None:
        if thr_adu < bg_median:
            _add_issue(issues, "threshold_adu_lt_bg_median")

    # mask stats
    mpix = _safe_int(r.get("mask_pixel_count"))
    mfrac = _safe_float(r.get("mask_coverage_frac"))
    ncomp = _safe_int(r.get("n_components"))
    lfrac = _safe_float(r.get("largest_component_frac"))

    if mpix is None or mpix < 0:
        _add_issue(issues, "mask_pixel_count_missing_or_negative")

    if mfrac is None:
        _add_issue(issues, "mask_coverage_frac_missing")
    else:
        if not _clamp01(mfrac):
            _add_issue(issues, "mask_coverage_frac_out_of_range")
        # if mask_pixel_count is 0, coverage should be 0 (or extremely close)
        if mpix is not None and mpix == 0 and mfrac > 1e-9:
            _add_issue(issues, "mask_coverage_nonzero_but_mask_pixel_count_zero")

    if ncomp is None or ncomp < 0:
        _add_issue(issues, "n_components_missing_or_negative")
    if lfrac is None:
        _add_issue(issues, "largest_component_frac_missing")
    else:
        if not _clamp01(lfrac):
            _add_issue(issues, "largest_component_frac_out_of_range")
        if ncomp is not None and ncomp == 0 and lfrac > 1e-9:
            _add_issue(issues, "largest_component_frac_nonzero_but_n_components_zero")

    # bbox json: if largest_component_frac > 0, bbox should be present (best-effort sanity)
    bbox = r.get("largest_component_bbox_json")
    if lfrac is not None and lfrac > 0:
        if bbox is None or (isinstance(bbox, str) and not bbox.strip()):
            _add_issue(
                issues, "largest_component_bbox_missing_when_largest_component_frac_pos"
            )
    ...
    if nanf is None:
        _add_issue(issues, "nan_fraction_missing")
    elif not _clamp01(nanf):
        _add_issue(issues, "nan_fraction_out_of_range")
    if inff is None:
        _add_issue(issues, "inf_fraction_missing")
    elif not _clamp01(inff):
...
def _rollup(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        g["_mask_frac"].append(_safe_float(r.get("mask_coverage_frac")))
        g["_ncomp"].append(_safe_float(r.get("n_components")))
        g["_largest_frac"].append(_safe_float(r.get("largest_component_frac")))
        ...
        out.append(
            {
                ...
                "avg_mask_coverage_frac": (
                    None
                    if avg(g["_mask_frac"]) is None
                    else round(avg(g["_mask_frac"]), 6)
                ),
                "avg_n_components": (
                    None if avg(g["_ncomp"]) is None else round(avg(g["_ncomp"]), 3)
                ),
                "avg_largest_component_frac": (
                    None
                    if avg(g["_largest_frac"]) is None
                    else round(avg(g["_largest_frac"]), 6)
                ),
            }
        )
    return out
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        "If set, outputs go into <run-dir>/nebula_mask_metrics/",
    )
    ap.add_argument(
        "--stamp",
        type=str,
