...
#     data/model_tests/masked_signal_metrics/test_results_<STAMP>/
#   Master run:
#     --run-dir data/model_tests/test_results_<STAMP>
#       writes into: <run-dir>/masked_signal_metrics/
#
# Exit codes:
#   0 PASS
#   2 FAIL
...
TABLE_NAME = "masked_signal_metrics"
WORKER_NAME = "masked_signal_worker"
...
SELECT
  ...
  m.nebula_minus_bg_adu,
  m.nebula_minus_bg_adu_s,
...
def _check_one_row(r: Dict[str, Any]) -> List[str]:
    ...
    # Pixel counts + fraction
    n_neb = _safe_int(r.get("nebula_pixel_count"))
    n_bg = _safe_int(r.get("bg_pixel_count"))
    frac = _safe_float(r.get("nebula_frac"))

    if n_neb is None or n_neb < 0:
        _add_issue(issues, "nebula_pixel_count_missing_or_negative")
    if n_bg is None or n_bg < 0:
        _add_issue(issues, "bg_pixel_count_missing_or_negative")

    if frac is None:
        _add_issue(issues, "nebula_frac_missing")
    elif not _clamp01(frac):
        _add_issue(issues, "nebula_frac_out_of_range")

    # If counts exist, frac should be consistent with counts (best-effort)
    if n_neb is not None and n_bg is not None and frac is not None:
        denom = n_neb + n_bg
        if denom > 0:
            frac_calc = float(n_neb) / float(denom)
            if abs(frac - frac_calc) > 0.02:  # loose tolerance
                _add_issue(issues, "nebula_frac_inconsistent_with_counts")

    # Medians/madstds sanity
    neb_med = _safe_float(r.get("nebula_median_adu"))
    bg_med = _safe_float(r.get("bg_median_adu"))
    neb_mad = _safe_float(r.get("nebula_madstd_adu"))
    bg_mad = _safe_float(r.get("bg_madstd_adu"))

    if neb_med is None:
        _add_issue(issues, "nebula_median_adu_missing")
    if bg_med is None:
        _add_issue(issues, "bg_median_adu_missing")
    if neb_mad is None:
        _add_issue(issues, "nebula_madstd_adu_missing")
    elif neb_mad < 0:
        _add_issue(issues, "nebula_madstd_adu_negative")
    if bg_mad is None:
    ...
    d = _safe_float(r.get("nebula_minus_bg_adu"))
    if d is None:
        _add_issue(issues, "nebula_minus_bg_adu_missing")
    else:
        if neb_med is not None and bg_med is not None:
            if abs(d - (neb_med - bg_med)) > 1e-3:
                _add_issue(issues, "nebula_minus_bg_adu_inconsistent_with_medians")

    # Per-second difference (if exptime present)
    d_s = _safe_float(r.get("nebula_minus_bg_adu_s"))
    if d_s is None:
        _add_issue(issues, "nebula_minus_bg_adu_s_missing")
    else:
        if d is not None and exptime_s is not None and exptime_s > 0:
            if abs(d_s - (d / exptime_s)) > 1e-3:
                _add_issue(issues, "nebula_minus_bg_adu_s_inconsistent_with_exptime")

    # SNR proxies should be >= 0
    snr = _safe_float(r.get("snr_proxy"))
    snr_s = _safe_float(r.get("snr_proxy_s"))
    if snr is None:
        _add_issue(issues, "snr_proxy_missing")
    elif snr < 0:
        _add_issue(issues, "snr_proxy_negative")
    if snr_s is None:
        _add_issue(issues, "snr_proxy_s_missing")
    elif snr_s < 0:
        _add_issue(issues, "snr_proxy_s_negative")
...
def _rollup(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        g["_nebula_frac"].append(_safe_float(r.get("nebula_frac")))
        g["_snr"].append(_safe_float(r.get("snr_proxy")))
        ...
        out.append(
            {
                ...
                "avg_nebula_frac": (
                    None
                    if avg(g["_nebula_frac"]) is None
                    else round(avg(g["_nebula_frac"]), 6)
                ),
                "avg_snr_proxy": (
                    None if avg(g["_snr"]) is None else round(avg(g["_snr"]), 6)
                ),
            }
        )
    return out
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        "If set, outputs go into <run-dir>/masked_signal_metrics/",
    )
    ap.add_argument(
        "--stamp",
        type=str,
    ...
    result: Dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "fail_if_worker_failed_pcnt": float(args.fail_if_worker_failed_pct),
